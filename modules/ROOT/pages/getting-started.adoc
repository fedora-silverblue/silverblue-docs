...
* Package layering: Most Fedora packages can be installed to the system with the help of package layering. By default the system operates in pure image mode, but package layering is useful for things like libvirt, drivers, etc.
...
As an example, Spotify can be installed by running the following command which can be found on Spotify's flathub page:

 $ flatpak install flathub com.spotify.Client

=== Flatpak command line

Additional details when it comes to using the flatpak command line interface can be found on the official http://docs.flatpak.org/en/latest/using-flatpak.html[Flatpak documentation].
