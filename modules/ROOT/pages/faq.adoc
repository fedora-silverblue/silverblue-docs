...
How can I play more videos in Firefox, like YouTube?::
    Firefox is included in the OS image for now. Until that changes, you can 
    install the Firefox https://flathub.org/apps/details/org.mozilla.firefox[Flatpak]. 
    Then simply run `rpm-ostree override remove firefox` to remove the RPM version from the base image.
    Alternatively, getting it to play videos works the same way as it does for the regular 
    Fedora Workstation: find a package with the needed codecs, and install it. The one 
    difference is that you use 
    `rpm-ostree install` instead of `dnf install`. 
    However, this will overlay quite a few packages that could be otherwise avoided.
+
TIP: The popular https://www.chromium.org/[Chromium] is also available as a https://flathub.org/apps/details/org.chromium.Chromium[Flatpak]

//To my knowledge Flash is EOL. Should we still include this portion of the guide considering that?
////
